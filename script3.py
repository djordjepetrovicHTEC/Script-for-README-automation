...
def commit_changes():
    ...
    os.system("git add .")
    os.system("git diff-index --quiet HEAD || git commit -m 'Update README.md'")
    os.system("git push")
...
for workflow_name, workflow_content in sections:
    ...
    if workflow_yaml_file in WORKFLOW_FILES:
        ...
        new_vars = [f'- `{var}`: (HERE)' for var in workflow_env_vars if var not in env_vars]
...
# Check if there are changes in the README file
git_diff_output = os.popen("git diff --name-only").read().strip()

if README_FILE in git_diff_output:
    commit_changes()
